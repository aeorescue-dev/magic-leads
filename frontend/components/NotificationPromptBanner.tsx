...
const DISMISSED_KEY = "garimpador.push.banner.dismissed";
...
export function NotificationPromptBanner() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setVisible(supported && permission === "default");
    }
  }, [hasUser, supported, permission]);
  ...
  const handleAllow = () => {
    subscribe().then(() => {
      // Only keep the banner once permission was granted or the flow completed.
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3">
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-emerald-300">
            {t("push.banner.title") || "Receba alertas de novas oportunidades"}
          </p>
          <p className="text-xs text-emerald-400/80 mt-0.5 max-w-md">
            {t("push.banner.text") ||
              "Ative as notificações para ser avisado assim que surgirem novas oportunidades."}
          </p>
      ...
      <div className="flex items-center gap-2 sm:ml-auto">
        ...
        >
          {t("push.banner.cta") || "Permitir notificações"}
        </button>
      </div>
    </div>
  );
